...
describe("SplitWiseGroup contract", function () {
    let owner: SignerWithAddress, member1: SignerWithAddress, member2: SignerWithAddress, member3: SignerWithAddress, member4: SignerWithAddress,
    ...
    before(async () => {
        ...
        splitWiseGroup = await SplitWiseGroup.deploy(ethers.utils.parseEther(REQUIRED_ETHER_AMOUNT), [
            member1.address,
            member2.address, member3.address, member4.address]);
    ...
    describe("Joining the group", function () {
        it("Should allow members to join the group with the required amount", async function () {
            await splitWiseGroup.connect(member1).join({value: ethers.utils.parseEther("1")});
            await splitWiseGroup.connect(member2).join({value: ethers.utils.parseEther("1")});
            await splitWiseGroup.connect(member4).join({value: ethers.utils.parseEther("1")});
        ...
        it("Should not allow members to join without sending the required amount", async function () {
            await expect(
                splitWiseGroup.connect(member3).join({value: ethers.utils.parseEther("0.5")})
        ...
        it("Should not allow members to join if already a member", async function () {
            await expect(
                splitWiseGroup.connect(member1).join({value: ethers.utils.parseEther("1")})
    ...
    describe("Remove members from the group", function () {
        it("Should allow the owner to remove an active member", async function () {
            ...
            const initialActiveMembersLength = await splitWiseGroup.getActiveMembersCount();
            ...
            const finalActiveMembersLength = await splitWiseGroup.getActiveMembersCount();
            expect(finalActiveMembersLength).to.equal(initialActiveMembersLength.sub(1));
        });

        it("Should not allow the owner to remove a non-member address", async function () {
            // Attempt to remove the owner (deployer)
            await expect(splitWiseGroup.removeMember(owner.address)).to.be.revertedWith("The address is not a member");

            // Ensure the owner is still an active member
            expect(await splitWiseGroup.isMember(owner.address)).to.be.false;
        });

        it("Should not allow non-owners to remove a member", async function () {
            // Attempt to remove member2 as a non-owner (member1)
            await expect(splitWiseGroup.connect(member1).removeMember(member2.address)).to.be.revertedWith(
    ...
    describe("Settling the group", function () {
        it("Should allow the owner to settle the group and distribute evenly", async function () {
            const initialBalanceMember1 = await ethers.provider.getBalance(member1.address);
            const initialBalanceMember2 = await ethers.provider.getBalance(member2.address);

            const tx = await splitWiseGroup.settleGroup();

            const finalBalanceMember1 = await ethers.provider.getBalance(member1.address);
            const finalBalanceMember2 = await ethers.provider.getBalance(member2.address);// Half of the contract balance

            await expect(tx).to.changeEtherBalance(member1, ethers.utils.parseEther("1.5"));
            await expect(tx).to.changeEtherBalance(member2, ethers.utils.parseEther("1.5"));
            expect(await splitWiseGroup.isSettled()).to.equal(true);
        });

        it("Should not allow settling the group multiple times", async function () {
            await expect(splitWiseGroup.settleGroup()).to.be.revertedWith("The group is closed");
        });
    });
